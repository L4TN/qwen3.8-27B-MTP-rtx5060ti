# Start IQ4_XS 45K Q4 - Extended context (15585 MiB 95.5%)
...
Write-Host "Starting IQ4_XS 45K Q4 on http://127.0.0.1:1234 - VRAM 15585 MiB (95.5%)" -ForegroundColor Green
Write-Host "Model: $MODEL | ctx 45056 | KV q4_0 | MTP n=3 | threads 6" -ForegroundColor DarkGray
& $LLAMA -m $MODEL --no-mmproj --device CUDA0 --spec-draft-device CUDA0 --gpu-layers-draft all --spec-type draft-mtp --spec-draft-n-max 3 --n-gpu-layers all --threads 6 --fit off --load-mode none --no-warmup --flash-attn on --ctx-size 45056 --parallel 1 --cache-type-k q4_0 --cache-type-v q4_0 --batch-size 512 --ubatch-size 512 --jinja --temp 1 --top-p 0.95 --top-k 20 --reasoning auto --reasoning-preserve --reasoning-effort medium --host 0.0.0.0 --port 1234 -lv 4
