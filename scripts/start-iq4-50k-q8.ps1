# Start IQ4_XS 50000 q8_0 - colapso 50K
...
Write-Host "Starting IQ4_XS 50000 q8_0 - colapso 50K on http://127.0.0.1:1234" -ForegroundColor Green
Write-Host "Model: $MODEL | ctx 50000 | KV q8_0 | MTP n=3 | threads 6 | beyond limite 45K - expect collapse" -ForegroundColor DarkGray
& $LLAMA -m $MODEL --no-mmproj --device CUDA0 --spec-draft-device CUDA0 --gpu-layers-draft all --spec-type draft-mtp --spec-draft-n-max 3 --n-gpu-layers all --threads 6 --fit off --load-mode none --no-warmup --flash-attn on --ctx-size 50000 --parallel 1 --cache-type-k q8_0 --cache-type-v q8_0 --batch-size 512 --ubatch-size 512 --jinja --temp 1 --top-p 0.95 --top-k 20 --reasoning auto --reasoning-preserve --reasoning-effort medium --host 127.0.0.1 --port 1234 -lv 4
