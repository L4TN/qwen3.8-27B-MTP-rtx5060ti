# Start IQ3_XXS 100000 q8_0 - limite 100K Q8 (revalidado)
...
Write-Host "Starting IQ3_XXS 100000 q8_0 - limite 100K Q8 on http://127.0.0.1:1234" -ForegroundColor Green
Write-Host "Model: $MODEL | ctx 100000 | KV q8_0 | MTP n=3 | threads 6 | limite 15452 MiB 84/57 - 110K colapsa" -ForegroundColor DarkGray
& $LLAMA -m $MODEL --no-mmproj --device CUDA0 --spec-draft-device CUDA0 --gpu-layers-draft all --spec-type draft-mtp --spec-draft-n-max 3 --n-gpu-layers all --threads 6 --fit off --load-mode none --no-warmup --flash-attn on --ctx-size 100000 --parallel 1 --cache-type-k q8_0 --cache-type-v q8_0 --batch-size 512 --ubatch-size 512 --jinja --temp 1 --top-p 0.95 --top-k 20 --reasoning auto --reasoning-preserve --reasoning-effort medium --host 0.0.0.0 --port 1234 -lv 4
